-- Search for all MPPs and TPPs based with an intended active ingredient starting with 'amox'
select
    v3_mpp_to_tpp.mppid,
...
where v3_ingredient_strength.substanceterm like 'amox%'
or v3_ingredient_strength.substanceterm like ' amox%'
or v3_mpp_to_tpp.mppterm like 'amox%'
or v3_mpp_to_tpp.mppterm like ' amox%'
or v3_mpp_to_tpp.tppterm like 'amox%'
or v3_mpp_to_tpp.tppterm like ' amox%'
;
...
select
    ...
    (SELECT term
        ...
        WHERE D.conceptId = hasDoseForm.destinationid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) AS formterm
...
select 
    ...
    (SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = ctpps.sourceid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1)
    as substitutectppterm
...
select distinct
    ...
    (SELECT term
        ...
        WHERE D.conceptId = hasDoseForm.destinationid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) AS doseformterm
...
select 
    ...
    (SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = hasMpuu.sourceid
        AND ADRS. acceptabilityid = 900000000000548007
    ...
    (SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = hasMpuu.typeid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) as hasmpuu_typeterm,
    hasMpuu.destinationid as mpuuid,
    (SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = hasMpuu.destinationid
        AND ADRS. acceptabilityid = 900000000000548007
    ...
    (SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = subRole.sourceid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) as sub_sourceterm,
    subRole.typeid as sub_typeid,
    (SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = subRole.typeid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) as sub_typeterm,
    subRole.destinationid as sub_destinationid,
    (SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = subRole.destinationid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) as sub_destinationterm
...
(SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = MPUUisMP.sourceid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) AS MPUU_term,
MPUUisMP.destinationid AS MP_id,
(SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = MPUUisMP.destinationid
        AND ADRS. acceptabilityid = 900000000000548007
        AND ADRS.active = 1) AS MP_term
