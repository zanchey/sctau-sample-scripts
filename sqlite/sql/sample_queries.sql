...
(SELECT term
        ...
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = referencedcomponentid
        AND ADRS.acceptabilityid = 900000000000548007
        AND ADRS.active = 1)
AS referencedComponentTerm,
targetComponentid,
(SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = targetComponentid
        AND ADRS.acceptabilityid = 900000000000548007
        AND ADRS.active = 1) 
AS targetComponentTerm
...
(SELECT term
        ...
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = refset_snapshot.referencedcomponentid
        AND ADRS.acceptabilityid = 900000000000548007
        AND ADRS.active = 1)
AS original_concept_PT,
crefset_snapshot.targetComponentid AS replacement_concept_ID,
(SELECT term
        FROM descriptions_snapshot AS D
        INNER JOIN language_refset_snapshot AS ADRS
        ON D.id = ADRS.referencedcomponentid
        WHERE D.conceptId = crefset_snapshot.targetComponentid
        AND ADRS.acceptabilityid = 900000000000548007
        AND ADRS.active = 1)
AS replacement_concept_term
